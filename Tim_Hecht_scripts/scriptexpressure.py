...
elementsList = {"B":.87, "N":.56,"H":.53,"C":.67,"O":.48,"Si":.88,
                ...
                "Nb_pv":1.98}
file = open('outputexpressure.dat','w')
file2 = open('outputpulay.dat','w')
...
def run():
    ...
    file2.write(folder)
    print folder
    proc3 = subprocess.Popen(['grep','-i','external','OUTCAR'],stdout=subprocess.PIPE)
    newstring = proc3.communicate()
    expressure = newstring[-2].split("\n")[-2].split()[3]
    pulay = newstring[-2].split("\n")[-2].split()[8]
    file.write('\t'+str(expressure)+'\n')
    file2.write('\t'+str(pulay)+'\n')
