...
use vector_matrix_utilities  !BCH took out "ONLY"
implicit none
private
public get_spaceGroup, get_spaceGroup_atomTypes, rm_3d_operations, make_primitive, get_lattice_pointGroup, get_pointgroup,&
      check_spaceGroup, does_mapping_exist, get_transformations,&
      bring_into_cell, find_site_equivalencies, int_symm_mats
...
! BCH: had to copy and rename Gus's routine to the one below because it could never find the one with the right name

subroutine get_pointgroup(aVecs, lattpg_op2, num_ops, eps_)
real(dp), intent(in):: aVecs(3,3)
!real(dp), intent(out):: lattpg_op(3,3,48)
real(dp), intent(out):: lattpg_op2(3*3*48)
...
!call writefloat3x3(aVecs,'Lattice vectors')
...
!Having trouble passing this to python, so I try: lattpg_op2
ielement = 0
do i = 1,3
	do j = 1,3
		do k = 1,num_ops
			ielement = ielement + 1
			lattpg_op2(ielement) = lattpg_op(i,j,k)
		enddo
	enddo
enddo

!call writefloat3x3N(lattpg_op,num_ops,'Symmetry operators')
...
real(dp), intent(in):: aVecs(3,3)
!real(dp), pointer:: lattpg_op(3,3,48)
real(dp), intent(out):: lattpg_op(3,3,48)
!real(dp), pointer:: lattpg_op(:,:,:)
real(dp), intent(in), optional:: eps_   ! "epsilon" for checking equivalence in 
                                        ! floating point arithmetic
...
END SUBROUTINE find_site_equivalencies

subroutine int_symm_mats(lat,imat,nop) !BCH
 !! Finds integer symmetry operators in the basis of the lattice vectors (which are columns of lat)
!    use num_types. nop is the number of nonzero operators
	real(dp), intent(in) :: lat(3,3)
    integer, intent(out) :: imat(3,3,48),nop
    real(dp) :: lattpg_op(3,3,48), symoptest(3,3,48), lat_inv(3,3), eps
    integer iop, irow, icol
    logical err
      eps = 1e-2
	  call get_lattice_pointGroup(lat,lattpg_op, eps)
	  nop = number_nonzero(lattpg_op)
      call writefloat3x3N(lattpg_op,nop,'Lattice point group operators')
	  call matrix_inverse(lat,lat_inv,err)
	  do iop = 1,nop !Transform symmetry operators to lattice basis (will be integer matrices)
	  	symoptest(:,:,iop) = matmul( lat_inv, matmul( lattpg_op(:,:,iop),lat) )
	  	do irow = 1,3
	  		do icol = 1,3
	  		  imat(irow,icol,iop)=nint(symoptest(irow,icol,iop))
	  		end do
	  	end do
	  end do
      call writeint3x3N(imat,nop,'Integer operators operators')
end subroutine int_symm_mats
