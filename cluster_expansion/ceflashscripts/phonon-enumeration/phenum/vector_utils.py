...
def map_enumStr_to_real_space(system_data,structure_data,minkowskiReduce):
        ...
        for z1 in range(a):
            for z2 in range(int((b*z1)/a), int(c+(b*z1)/a)):
                for z3 in range(int(z1*(d-(e*b)/c)/a+(e*z2)/c), int(f+z1*(d-(e*b)/c)/a+(e*z2)/c)):
                    ...
                    #     #from msg import err
                    #     print("Problem with basis atoms in map_enpStr_to_real_space...")
                    #     exit()
                    ...
                    if not allclose(greal,g,rtol=eps,atol=eps): #pragma: no cover
                        #from msg import err
                        print("map2G didn't work in map_enumStr_to_real_space")
                        exit()
    ...
    if ic != n*n_d: #pragma: no cover
        #from msg import err
        print("ERROR: map_enumStr_to_real_space: Didn't find the correct # of basis atoms")
        exit()
...
def _minkowski_reduce_basis(IN,eps):
    ...
    if not _minkowski_conditions_check(OUT,eps): #pragma: no cover
        #from msg import err
        print("ERROR in minkowski_reduce_basis: Minkowski conditions not met."
...
def _minkowski_conditions_check(basis,eps):
    ...
    if linalg.norm(b1) > (linalg.norm(b2) + eps):
        ...
        print("Minkowski_condition 1 failed: b1 > b2")

    if linalg.norm(b2) > (linalg.norm(b3) + eps):
        minkowski_check = False
        print("Minkowski_condition 2 failed: b2 > b3")

    if linalg.norm(b2) > (linalg.norm([b1[i]+b2[i] for i in range(len(b1))])+eps):
        minkowski_check = False
        print("Minkowski_condition 3 failed: b2 > b1+b2")

    if linalg.norm(b2) > (linalg.norm([b1[i]-b2[i] for i in range(len(b1))])+eps):
        minkowski_check = False
        print("Minkowski_condition 4 failed: b2 > b1-b2")

    if linalg.norm(b3) > (linalg.norm([b1[i]+b3[i] for i in range(len(b1))])+eps):
        minkowski_check = False
        print("Minkowski_condition 5 failed: b3 > b1+b3")

    if linalg.norm(b3) > (linalg.norm([b3[i]-b1[i] for i in range(len(b1))])+eps):
        minkowski_check = False
        print("Minkowski_condition 6 failed: b3 > b3-b1")

    if linalg.norm(b3) > (linalg.norm([b2[i]+b3[i] for i in range(len(b2))])+eps):
        minkowski_check = False
        print("Minkowski_condition 7 failed: b3 > b2+b3")

    if linalg.norm(b3) > (linalg.norm([b3[i]-b2[i] for i in range(len(b2))])+eps):
        minkowski_check = False
        print("Minkowski_condition 8 failed: b3 > b3-b2")

    if linalg.norm(b3) > (linalg.norm([b1[i]+b2[i]+b3[i] for i in range(len(b2))])+eps):
        minkowski_check = False
        print("Minkowski_condition 9 failed: b3 > b1+b2+b3")

    if linalg.norm(b3) > (linalg.norm([b1[i]-b2[i]+b3[i] for i in range(len(b2))])+eps):
        minkowski_check = False
        print("Minkowski_condition 10 failed: b3 > b1-b2+b3")

    if linalg.norm(b3) > (linalg.norm([b1[i]+b2[i]-b3[i] for i in range(len(b2))])+eps):
        minkowski_check = False
        print("Minkowski_condition 11 failed: b3 > b1+b2-b3")

    if linalg.norm(b3) > (linalg.norm([b1[i]-b2[i]-b3[i] for i in range(len(b2))])+eps):
        minkowski_check = False
        print("Minkowski_condition 12 failed: b3 > b1-b2-b3")
...
def _reduce_C_in_ABC(A,B,C,eps):
    ...
    if not allclose(dot(T,cross(A,B)),0,atol=eps,rtol=eps): #pragma: no cover
        #from msg import err
        print("{0} Projection of C into A,B plane failed".format(str(dot(T,cross(A,B)))))
    ...
    if idx == 0:
        ...
        #from msg import err 
        print("Case failed in reduce_C_in_ABC Lattice coordinates in the A,B plane: {0}".format(' '.join([str(i) for i in LC])))
    ...
    for i in range(3):
        for j in range(3):
            if abs(temp[i][j] - int(round(temp[i][j]))) > eps: #pragma: no cover
                #from msg import err
                print("Lattice was not preserved in reduce_C_in_ABC")
                exit()
...
def _gaussian_reduce_two_vectors(U,V,eps):
    ...
    while not done:
        if it > 10: # pragma: no cover
            #from msg import err
            print("gaussian_reduce_two_vectors failed to converge in 10 iterations")
            exit()
