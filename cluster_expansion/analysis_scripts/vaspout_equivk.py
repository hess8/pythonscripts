...
outfile.write('n,energy,Natoms, energy/atom,el converged, el steps, NIBZ,cputime(min\n')
...
en_per_atom = [str(float(energies[i])/dets[i]) for i in range(len(dets))]
ns = [str(float(ms[i]*dets[i])) for i in range(len(dets))]

for i in range(len(names)):
    linei = names[i]+','+energies[i]+','+str(dets[i])+','+en_per_atom[i]+','+elconverge[i]+','+elsteps[i]+','+NkIBZ[i]+','+ str(round(float(cputime[i])/60,2))+'\n'        
    outfile.write(linei)    
outfile.close() 
